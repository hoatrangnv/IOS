...
//    [self addButton:@"ic_action_email" selector:@selector(suKienBamNutGuiMail) atSide:1];
    ...
    UISegmentedControl *control = [[UISegmentedControl alloc] initWithItems:[NSArray arrayWithObjects:@"Nhận tiền", @"Thanh toán", nil]];
    [control addTarget:self action:@selector(suKienChonOptionSaoKe:) forControlEvents:UIControlEventValueChanged];
    ...
    [self.lvHistory setTableFooterView:[[UIView alloc] initWithFrame:CGRectZero]];
    ...
    nKieuLaySaoKe = GET_ALL_INCOMMING_INQUIRY;
    [self suKienChonVi:_mbtnVi];
    
    UIButton * button = [UIButton buttonWithType:UIButtonTypeCustom];
    button.frame = CGRectMake(0, 0, 30, 30);
    [button setImage:[UIImage imageNamed:@"ic_question_32"]forState:UIControlStateNormal];
...
- (void)suKienChonOptionSaoKe:(UISegmentedControl *)sender {
    NSLog(@"DucNT_SaoKeViewController - %s - Click : %ld", __FUNCTION__, (long)sender.selectedSegmentIndex);
    if (sender.selectedSegmentIndex == 0 && nKieuLaySaoKe != GET_ALL_INCOMMING_INQUIRY) {
        nKieuLaySaoKe = GET_ALL_INCOMMING_INQUIRY;
    } else if (nKieuLaySaoKe != GET_ALL_OUTCOMMING_INQUIRY) {
        nKieuLaySaoKe = GET_ALL_OUTCOMMING_INQUIRY;
    }
    indexSaoKe = 0;
    mTrangThaiDuocPhepKetNoi = YES;
...
-(void)khoiTaoKetNoiLaySaoKeVi
{
    if(mTrangThaiDuocPhepKetNoi)
    {
        ...
        [GiaoDichMang ketNoiLaySaoKeVi:nThoiGianBatDau thoiGianKetThuc:nThoiGianKetThuc viTribatDau:indexSaoKe gioiHan:limit kieuLay:nKieuLaySaoKe searchTextQR:_exSearch.text noiNhanKetQua:self];
        
//        [GiaoDichMang ketNoiLaySaoKeVi:nThoiGianBatDau
//                       thoiGianKetThuc:nThoiGianKetThuc
//                           viTribatDau:indexSaoKe
//                               gioiHan:limit
//                               kieuLay:nKieuLaySaoKe
//                         noiNhanKetQua:self];
    }
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.lbSoTien.text = [NSString stringWithFormat:@"%@ đ", [Common hienThiTienTe:[item.amount doubleValue]]];
    cell.lbSoTien.textColor = [UIColor blackColor];
    
    cell.lblToAcc.text = item.toAcc;
    
    cell.lbNoiDung.text = [item layNoiDung];
    cell.lbNoiDung.textColor = [UIColor blackColor];
    
    cell.lbThoiGian.text = [NSString stringWithFormat:@"%@ %@", [item layNgayThangChuyenTien], [item layNgayGioChuyenTien]];
