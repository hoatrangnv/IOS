...
-(void)chuyenFormVeDangNhap:(NSNotification *)notification
{
    if([[notification name] isEqualToString:@"XAC_THUC_OTP_THANH_CONG"])
    {
        ...
        {
            [self luuMatKhauDangNhap:edtMatKhau.text cuaTaiKhoanViMASS:app.mThongTinTaiKhoanVi.walletLogin];
            NSDictionary *dictDN = @{app.mThongTinTaiKhoanVi.walletLogin : app.mThongTinTaiKhoanVi.companyCode};
            [DucNT_LuuRMS luuThongTinDangNhap:KEY_DANG_NHAP_DOANH_NGHIEP value:[dictDN JSONString]];
        }
        else
        {
            [self luuMatKhauDangNhap:edtMatKhau.text cuaTaiKhoanViMASS:edtSoDienThoai.text];
        }
        //        [DucNT_LuuRMS luuThongTinDangNhap:KEY_LOGIN_PASS value:edtMatKhau.text];
