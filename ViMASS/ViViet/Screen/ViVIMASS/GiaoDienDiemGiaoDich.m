...
@interface GiaoDienDiemGiaoDich (){
...
- (void)viewDidLoad {
    ...
    NSLog(@"%s - width : %f - height : %f", __FUNCTION__, width, height);
    [self.view setFrame:CGRectMake(0, 0, 400, height)];
    [self.view setNeedsDisplay];
}

- (void)labelDiaDiemClick{
    NSLog(@"%s - %s : click!!!!", __FILE__, __FUNCTION__);
...
- (void)khoiTaoNavigationBar{
    ...
    textFeildSearch = [[ExTextField alloc] initWithFrame:CGRectMake(0, 0, 220, 30)];
...
- (void)suKienSwitchMap:(id)btn{
    UIButton *btnTemp = (UIButton *)btn;
    if (self.tableDiaDiem.isHidden == NO) {
        [btn setImage:[UIImage imageNamed:@"icon_switch_list.png"] forState:UIControlStateNormal];
        if (SYSTEM_VERSION_LESS_THAN(@"11")) {
        }
        else {
            [btnTemp.widthAnchor constraintEqualToConstant:34].active = YES;
            [btnTemp.heightAnchor constraintEqualToConstant:34].active = YES;
...
- (void)scrollViewWillBeginDecelerating:(UIScrollView *)scrollView{
    if ([scrollView isEqual:self.tableDanhSachTinhThanh]) {
        if (![self.tableSubTinhThanh isHidden]) {
            [self.tableSubTinhThanh setHidden:YES];
        }
    }
}
...
- (void)locationManager:(CLLocationManager *)manager didChangeAuthorizationStatus:(CLAuthorizationStatus)status{
    NSLog(@"GiaoDienDiemGiaoDich : didChangeAuthorizationStatus : status : %d", status);
...
//            NSLog(@"%s - %s : lat : %f - lng : %f", __FILE__, __FUNCTION__, item.latitude, item.longtitude);
