...
- (void)viewDidLoad {
    ...
    [button addTarget:self action:@selector(suKienChonThemQRDonVi:) forControlEvents:UIControlEventTouchUpInside];
...
//        NSLog(@"%s - arrTemp.count : %ld", __FUNCTION__, arrTemp.count);
        if (arrTemp.count > 0) {
            [cell.imgvAvatar sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"https://vimass.vn/vmbank/services/media/getImage?id=%@", [arrTemp firstObject]]] placeholderImage:[UIImage imageNamed:@"icon_danhba"]];
        }
