...
-(CGFloat)idealOffsetForView:(UIView *)view withSpace:(CGFloat)space
{
    ...
    UIEdgeInsets edge = [self contentInsetForKeyboard];
    
    CGFloat off = rect.origin.y + rect.size.height - space + 10 + self.addtionOffset;
    if (off < 0)
        off = 0;
    if (off + self.frame.size.height - space > self.contentSize.height)
    {
        // WTF is this. This should be:
//        off = self.contentSize.height - self.contentSize.height + space;
        NSLog(@"%@", NSStringFromUIEdgeInsets(self.contentInset));
        off = self.contentSize.height + edge.bottom - self.frame.size.height;
    }
    return off;
